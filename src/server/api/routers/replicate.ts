  ...
  startTrainingModel: protectedProcedure.mutation(
    async ({ ctx: { prisma, session } }) => {
      ...
      if (user.modelTrainingLimit === 0) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "only one model can be trained from a single account",
        });
      }

      // check if zip exists in s3 bucket
      let zippedDataSignedUrl;
      try {
        zippedDataSignedUrl = await getSignedUrl(
          s3,
        ...
        const { data } = await axios.post(
          "https://dreambooth-api-experimental.replicate.com/v1/trainings",
          {
            input: {
              ...
              instance_data: zippedDataSignedUrl,
              max_train_steps: 2000,
            },
            model: `ashamn/${session.user.id}`,
            trainer_version:
